...
public class AstroTest {
    ...
    public void jd() {
        ...
        gc.set(1983, 1, 18, 7, 12, 0);
        assertEquals(2445352.8, Astro.julian_date(gc), 0);
    }

    @Test
    public void sideralTime() {
        double theta = Astro.sideral_time(1982, 1, 1, 0);
        Degree hms = Astro.deg2sex(theta);
        assertEquals("mean sideral time Greenwich h", 6, hms.d);
        assertEquals("mean sideral time Greenwich min", 41, hms.m);
        assertEquals("mean sideral time Greenwich sec", 17.3, hms.s,0.1);
        double local = theta + 11.60833 / 15;
        hms = Astro.deg2sex(local);
        assertEquals("mean sideral time Munich h", hms.d, 7);
        assertEquals("mean sideral time Munich min", hms.m, 27);
        assertEquals("mean sideral time Munich sec", hms.s, 43.3, 0.1);
    }
    ...
    public void sphe2cart() {
        double beta = 60; double lam = 120; double r = 5;
    ...
    public void cart2sphe() {
        double x = -1.0; double y = -2.5; double z = 0.5;
